...
def test_packer_summary_from_pending_state_excludes_prefetched_batch():
    state = {
    ...
    result = packer_summary_from_state(state)

    assert result["batches_emitted"] == 4
    assert result["packing_efficiency"] == 0.9
