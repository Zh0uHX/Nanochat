...
- `sft_packing_d26_a800_30step/`: reviewed single-run target-scale pilot for all
  four strategies from the same step-14,889 base checkpoint. `summary.json`
  validates common controls and raw-file hashes; `dataset_manifest.json`
  records HF revisions and local/tokenizer hashes. The pilot supports
  content-per-fixed-budget utilization, not causal wall-clock speedup or a
  definitive quality conclusion.
